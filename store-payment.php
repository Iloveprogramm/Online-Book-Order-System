...
if(!preg_match("/^(0[1-9]|1[0-2])\/([0-9]{2})$/", $expiry)) {
    echo "Invalid expiry format. Please use MM/YY.";
    exit;
}

// Validate cvc
if(strlen($cvc) !== 3 || !ctype_digit($cvc)) {
    echo "Invalid CVC. It must be a 3-digit number. " ;
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (is_null($itemsHTML) || is_null($totalAmount)) {
        echo "Essential data is missing. Please check your input and try again.";
        exit;  
    }    
    ...
    if ($stmtOrder->execute()) {
        ...
        echo "Error in storing order: " . $stmtOrder->error;
    }
